...
import { FONT_MONO, FONT_SERIF, DESK, COLOR, GAP, FONT_SIZE } from "../../constants/theme.js";
    ...
    <DeskRowShell {...dr} onViewAll={onViewAll}
      renderInfo={() => (
        <div onClick={() => dr.handOpen ? (dr.focusPhase ? null : dr.setHandOpen(false)) : onViewAll()} style={{
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: GAP.sm, marginBottom: GAP.sm, whiteSpace: "nowrap" }}>
            {s && <SIcon s={s} size={FONT_SIZE.xxl} />}
            <span style={{ fontFamily: FONT_SERIF, fontSize: 26, fontStyle: "italic", fontWeight: 400, color: displayColor, letterSpacing: -0.4 }}>{displayLabel}</span>
            <span style={{ fontFamily: FONT_MONO, fontSize: 10, color: COLOR.sub, marginLeft: GAP.xs, letterSpacing: 1.2, textTransform: "uppercase" }}>{dr.count} items</span>
          </div>
